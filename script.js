class Example extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "logo",
      `https://play.rosebud.ai/assets/CLICK-TO-START-7-24-2024 (1).png?kote`
    );
    // Removed 'red' image load as it's no longer needed
  }

  create() {
    this.add.image(400, 300, "sky");

    // Removed particle emitter code

    const logo = this.physics.add.image(400, 100, "logo");

    // Make the logo 50% smaller
    logo.setScale(0.6);
    ...
    // Removed emitter.startFollow(logo);
...
class NewScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "clapper",
      "https://play.rosebud.ai/assets/clapper-board.png?DdiR"
    );
    this.load.image(
      "camera",
      "https://play.rosebud.ai/assets/camcorder.png?8gqg"
    );
  ...
  create() {
    ...
    const backButton = this.add
      .text(400, 550, "Back to main", { fontSize: "24px", fill: "#fff" })
      .setOrigin(0.5)
      .setInteractive()
      .on("pointerdown", () => {
    ...
    this.trophyText = this.add.text(700, 550, "Trophies: 0/3", {
      fontSize: "24px",
      fill: "#fff",
    });
  }
